...
function CakeDesigner({ onCreate, onClose }) {
  ...
  const handleCreate = () => {
    const design = { cakeBase, filling, frosting, decoration };
    onCreate(design);
  };
  ...
  return (
    <div className="cake-designer-container">
      ...
      <button className="create-btn" onClick={handleCreate}>Create Design</button>
    </div>
  );
}
